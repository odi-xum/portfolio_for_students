...
def _base_query(db: Session, student_ids, status_filter, date_from, date_to):
    q = db.query(Event).filter(Event.student_id.in_(student_ids))
...
# ============================================================================
def _sheet_summary(db: Session, ws, students, status_filter, date_from, date_to):
    ws.title = 'Сводка'
    headers = ['№', 'ФИО', 'Логин', 'Группа', 'Всего', 'Одобрено',
               'На проверке', 'Отклонено', 'Ср. балл']
    ...
    for idx, s in enumerate(students, 1):
        student_base = _base_query(db, [s.id], status_filter, date_from, date_to)

        total = student_base.count()
        approved = student_base.filter(Event.status == 'approved').count()
        pending = student_base.filter(Event.status == 'pending').count()
        rejected = student_base.filter(Event.status == 'rejected').count()
        scored = student_base.filter(Event.status == 'approved', Event.score.isnot(None)).all()
        avg = round(sum(e.score for e in scored) / len(scored), 2) if scored else 0

        vals = [idx, _fio(s), s.username, s.group_name or '—',
                total, approved, pending, rejected, avg]
...
# ============================================================================
def _sheet_details(db: Session, ws, students, status_filter, date_from, date_to):
    ...
    q = _base_query(db, ids, status_filter, date_from, date_to)
...
# Sheet 3: Графики
# ============================================================================
def _sheet_charts(db: Session, ws, students, date_from, date_to):
    ws.title = 'Графики'
    ids = [s.id for s in students]

    def _base():
        q = db.query(Event).filter(Event.student_id.in_(ids))
        if date_from:
            q = q.filter(Event.created_at >= _parse_dt(date_from))
        if date_to:
            q = q.filter(Event.created_at <= _parse_dt(date_to, True))
    ...
    labels = ['Одобрено', 'На проверке', 'Отклонено']
    keys = ['approved', 'pending', 'rejected']
    colors = ['A9B665', 'D8A657', 'EA6962']
    bq = _base()
...
# ПУБЛИЧНАЯ ФУНКЦИЯ
# ============================================================================
def generate_report(db: Session, students, include_charts=True, include_details=True,
    ...
    """
    wb = Workbook()
    _sheet_summary(db, wb.active, students, status_filter, date_from, date_to)

    if include_details:
        _sheet_details(db, wb.create_sheet(), students, status_filter, date_from, date_to)

    if include_charts:
        _sheet_charts(db, wb.create_sheet(), students, date_from, date_to)
